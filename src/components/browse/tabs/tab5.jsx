...
const Tab5 = ({ data }) => {
  return (
    <table className="w-full p-8 text-xl mt-4">
      ...
      <tbody>
        {
          data.map((dt, id) => 
            <tr className="h-36" key={`replacement-${id}`}>
              <td><img src={dt.image} alt="" className="w-full h-36 p-4 object-contain"/></td>
              <td className="text-center">{dt.description}</td>
              <td className="text-center">{dt.id}</td>
              <td className="text-center">{dt.price}</td>
              <td className="px-4"><input type="number" min={0} className="!w-16 outline-none border-b border-normalGray"/></td>
              <td className="w-24 h-36 flex justify-center items-center"><button className="bg-[#E6E6E6] text-[#505050] border border-[#505050] rounded-full text-2xl p-2"><BsCartPlus /></button></td>
            </tr>
          )
        }
      </tbody>
