  ...
  return (
    <section className="w-full h-full z-10 relative">
      ...
      >
        {slides.map((slide, id) => (
          <div
            className="w-full h-full grid grid-cols-4 relative"
            key={`homeslide-${id}`}
          >
        ...
        <div className="w-full h-full grid lg:grid-cols-6 grid-cols-3 border-2 border-normalGray">
          {navs.map((nav, id) => (
            <Link to={nav.url} key={`nav-${id}`}>
              <div className="text-black lg:text-3xl md:text-2xl sm:text-xl cursor-pointer h-full w-full flex items-center bg-white border-x border-normalGray">
